...
from ndk import sdk_common_pb2
...
def Remove_Telemetry(js_paths):
    ...
    telemetry_response = telemetry_stub.TelemetryDelete(
        request=telemetry_del_request, metadata=metadata
    )
    return telemetry_response
...
class BGPMonitoringThread(Thread):
    ...
    def run(self):
        ...
        def handle_bgp_keepalive(packet, is_ping):
            ...
            logging.info(
                f"handle_bgp_keepalive is_ping={is_ping} ts={ts} peer={peer} len={len(packet)} if={packet.sniffed_on}"
            )
            if peer in self.state_per_peer:
                s = self.state_per_peer[peer]
                if is_ping:
                    s.update(
                        {"time": packet.time, "ts": tsval, "count": s["count"] + 1}
                    )
                elif tsecr == s["ts"]:
                    ...
                    if "last_ttl" in s and s["last_ttl"] != ip.ttl:
                        s["ttl_changes"] = (
                            1 if "ttl_changes" not in s else (s["ttl_changes"] + 1)
                        )
                    s["last_ttl"] = ip.ttl
        ...
        def check_for_bgp_keepalive(packet):
            if (
                len(packet) == 85 and packet.sniffed_on == "gateway"
            ):  # outgoing PING BGP keep-alive
                ...
                if obj.HasField("config") and obj.config.key.js_path == ".commit.end":
                    ...
                    if not hasattr(state, "bgpthread"):
                        if state.admin_state == "enable":
                            state.bgpthread = BGPMonitoringThread(
                                state.network_instance
                            )
                            state.bgpthread.start()
                    elif state.admin_state == "disable":
                        logging.info(
                            "Stopping BGP keep-alive sniff thread after next packet..."
                        )
